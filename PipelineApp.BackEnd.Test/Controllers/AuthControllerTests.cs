...
namespace PipelineApp.BackEnd.Test.Controllers
{
    ...
    public class AuthControllerTests : ControllerTests<AuthController>
    {
        ...
        {
            [Fact]
            public async void ReturnsServerErrorWhenUnexpectedErrorOccurs()
            {
                // Arrange
                var model = new RefreshTokenRequest
                {
            ...
            [Fact]
            public async void ReturnsOkWhenNoExceptionsThrown()
            {
                // Arrange
                var model = new RefreshTokenRequest()
                {
        ...
        public class Register : AuthControllerTests
        {
            ...
            public async Task ReturnsBadRequestWhenRegistrationIsInvalid()
            {
                ...
                _mockAuthService.Verify(s => s.Signup(It.IsAny<UserEntity>(), It.IsAny<string>(), _mockUserManager.Object), Times.Never);
                _mockAuthService.Verify(s => s.AddUserToRole(It.IsAny<UserEntity>(), It.IsAny<string>(), _mockUserManager.Object), Times.Never);
            }
            ...
            public async Task ReturnsBadRequestWhenUserInformationExists()
            {
                // Arrange
                var exception = new InvalidRegistrationException(new List<string> { "error1", "error2" });
                _mockAuthService.Setup(s => s.AssertUserInformationDoesNotExist("me@me.com", _mockUserManager.Object))
                    .Throws(exception);
            ...
            public async Task ReturnsBadRequestWhenUserCreationFails()
            {
                // Arrange
                var exception = new InvalidRegistrationException(new List<string> { "error1", "error2" });
                _mockAuthService.Setup(s => s.Signup(It.Is<UserEntity>(u => u.Email == "me@me.com"), "my-password", _mockUserManager.Object))
                    .Throws(exception);
            ...
            {
                // Arrange
                _mockAuthService.Setup(s => s.Signup(It.IsAny<UserEntity>(), It.IsAny<string>(), _mockUserManager.Object))
                    .Throws<NullReferenceException>();
            ...
            {
                // Arrange
                var createdUser = new UserEntity { Id = Guid.NewGuid() };
                _mockAuthService.Setup(s => s.Signup(It.IsAny<UserEntity>(), It.IsAny<string>(), _mockUserManager.Object))
